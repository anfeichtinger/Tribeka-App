...
import 'package:tribeka/util/latin_1_transformer.dart';
...
class Session {
  ...
  Future<void> _post(String url, Map<String, String> data) async {
  ...
  Future<void> autoLogin(BuildContext _context) async {
  ...
  Future<void> _loginOnExpiration() async {
    final _email = await _storage.read(key: 'email');
    final _password = await _storage.read(key: 'password');
    try {
      await _post(baseURL + "login/", {
